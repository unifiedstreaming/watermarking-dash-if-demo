...
sub vcl_backend_fetch {
  ...
  std.log("bereq.http.Representation-ID: " + bereq.http.Representation-ID);
...
sub vcl_backend_response {
  std.log("**** Starts vcl_backend_response ****");
      ...
      && bereq.http.x-state != "valid")
    {
        ...
        # # Remove the Media RepresentationID and set it as a HTTP request Header
        # # This will gurantee ONE single filecar for all the representations of
        # # the same media segment.
        # # Set the RepresentationID as as HTTP request Header
        set bereq.http.Representation-ID = regsub(bereq.http.X-side-car, ".*\/.*?\/.*=(\d+)-\d+.json$",  "\1");
        std.log("bereq.http.Representation-ID: " + bereq.http.Representation-ID );
        # Remove the RepresentatinID from the sidecar URL
        set bereq.http.X-side-car = regsub(bereq.http.X-side-car, "(.*\/.*?\/.*=)\d+(-\d+.json$)", "\1\2");
        std.log("bereq.http.X-side-car: " + bereq.http.X-side-car);
        ...
        set beresp.http.Representation-ID = bereq.http.Representation-ID;
    }
...
sub vcl_deliver {
  ...
  std.log("resp.http.X-Cache: " + resp.http.X-Cache);
  ...
  if (resp.http.x-state == "backend_check")
  {
    ...
    # Set the request header of the RepresentationId
    set req.http.Representation-ID = resp.http.Representation-ID;
    return (restart);
  }
  ...
  set resp.http.file = "watermarking_live_setup_2_one_sidecar_for_all_reps.vcl";
