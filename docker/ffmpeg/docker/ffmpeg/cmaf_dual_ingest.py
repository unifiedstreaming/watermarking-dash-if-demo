...
# env options
if "PUB_POINT_URI_A" and "PUB_POINT_URI_B" in os.environ:
    pub_point_uri_a = os.environ["PUB_POINT_URI_A"]
    pub_point_uri_b = os.environ["PUB_POINT_URI_B"]
else:
    logger.critical("must set PUB_POINT_URI_A and PUB_POINT_URI_B")
    exit(1)
...
for video in tracks["video"]:
    ...
    command.append([
        ...
        ALL_TRACK_OPTS,
        f"[f=mp4:movflags={MOVFLAGS}]{pub_point_uri_a}/Streams(video_{video['width']}x{video['height']}_{video['bitrate']}.cmfv)|[f=mp4:movflags={MOVFLAGS}]{pub_point_uri_b}/Streams(video_{video['width']}x{video['height']}_{video['bitrate']}.cmfv)"
    ])
...
for audio in tracks["audio"]:
    ...
    command.append([
        ...
        ALL_TRACK_OPTS,
        f"[f=mp4:movflags={MOVFLAGS}]{pub_point_uri_a}/Streams(audio_{audio['language']}_{audio['bitrate']}.cmfa)|[f=mp4:movflags={MOVFLAGS}]{pub_point_uri_b}/Streams(audio_{audio['language']}_{audio['bitrate']}.cmfa)"
    ])
